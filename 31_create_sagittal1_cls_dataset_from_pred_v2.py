...
def get_args():
    ...
    parser.add_argument("--dirname", type=str, default="sagittal1_cls_all_dataset")
...
def main():
    ...
    if mode == "val":
        dfs = [pd.read_csv(f"sagittal1_val_keypoint_preds_fold{i}.csv") for i in range(5)]
        df = pd.concat(dfs, axis=0)
        output_dir = root_dir.joinpath(args.dirname)
    else:
        df = pd.read_csv("sagittal1_test_keypoint_preds.csv")
        output_dir = Path(f"sagittal1_cls_test_dataset")
    ...
    for series_id, sub_df in tqdm(df.groupby("series_id")):
        for side in ["left", "right"]:
            ...
            for row in side_df.itertuples(index=False):
                study_id, series_id, instance_number, side, part_id, x, y = row
            ...
            for part_id, (x, y) in enumerate(keypoints):
                ...
                if mode == "val":
                    target_column_name = f"{side}_neural_foraminal_narrowing_{level}"
                    target = train_df[train_df["study_id"] == study_id][target_column_name].values[0]
                else:
                    target = "Normal/Mild"

                cropped_img = crop_img(img, x, y, scale)
                filename = f"{study_id}_{series_id}_{part_id}_{instance_number}_{side}.npz"
                ...
                rows.append([study_id, series_id, filename, instance_number, side, level, target])

    df = pd.DataFrame(rows, columns=["study_id", "series_id", "filename", "instance_number", "side", "level", "target"])
