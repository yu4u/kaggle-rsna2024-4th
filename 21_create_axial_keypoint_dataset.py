...
def get_args():
    ...
    parser.add_argument("--dirname", type=str, default="axial_keypoint_dataset")
...
def main():
    ...
    train_df = pd.read_csv(root_dir.joinpath("train.csv"))
    ...
    for series_id, sub_df in tqdm(coord_df.groupby("series_id")):
        ...
        for _, row in sub_df.iterrows():
            ...
            """
            for i in range(3):
                dcm_path = Path(f"input/train_images/{study_id}/{series_id}/{instance_number - 1 + i}.dcm")

                if not dcm_path.exists():
                    dcm_path = Path(f"input/train_images/{study_id}/{series_id}/{instance_number}.dcm")
                    print(f"not found: {dcm_path}")

                img = get_img_from_dcm(dcm_path, normalize=False)
                imgs.append(img)
            """
            ...
            left_flag = 0 if row["condition"].startswith("Left") else 1
            x = row["x"] / w * img_size
            y = row["y"] / h * img_size
            img_filename = f"{study_id}_{series_id}_{instance_number}_{left_flag}_img.npy"
            ...
            mask_filename = f"{study_id}_{series_id}_{instance_number}_{left_flag}_mask.png"
            ...
            target = train_df[train_df["study_id"] == study_id][target_column_names].values[0]

            rows.append([study_id, series_id, img_filename, left_flag, *target])

    df = pd.DataFrame(rows, columns=["study_id", "series_id", "filename", "left_flag"] + target_names)
