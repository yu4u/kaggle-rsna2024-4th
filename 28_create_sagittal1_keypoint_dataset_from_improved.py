...
def get_args():
    ...
    parser.add_argument("--dirname", type=str, default="sagittal1_keypoint_dataset")
...
def main():
    ...
    for series_id, sub_df in tqdm(coord_df.groupby("series_id")):
        ...
        for side in ["Left", "Right"]:
            side_df = sub_df[sub_df["condition"].str.startswith(side)]
            instance_number = int(side_df["instance_number"].values.mean())
            ...
            left_flag = 1 if side == "Left" else 0
            ...
            side_df = side_df.sort_values("level")
            masks = []
            ...
            mask_filename = f"{study_id}_{series_id}_{instance_number}_{left_flag}_mask.npy"
